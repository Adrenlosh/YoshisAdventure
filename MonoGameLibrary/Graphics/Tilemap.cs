...
public class Tilemap
{
    ...
    public Tilemap(Tileset tileset, int columns, int rows)
    {
        ...
        {
            // 默认初始化为 tileset 的第 0 个 Tile，可根据实际需求调整
            _tiles[i] = new Tile(_tileset.GetTile(0).Region, false, 0);
        }
    }
    ...
    {
        return _tileset.GetTile(_tiles[index].TileID);
    }
    ...
    public void Draw(SpriteBatch spriteBatch)
    {
        for (int i = 0; i < Count; i++)
        {
            ...
            TextureRegion tile = _tileset.GetTile(tileSetIndex.TileID).Region;
    ...
    public static Tilemap FromFile(ContentManager content, string filename)
    {
        ...
        using (Stream stream = TitleContainer.OpenStream(filePath))
        {
            using (XmlReader reader = XmlReader.Create(stream))
            {
                ...
                XElement blockingTilesElement = root.Element("BlockingTiles");
                int[] blockingTiles = Array.ConvertAll(blockingTilesElement.Value.Trim().Split(" ", StringSplitOptions.RemoveEmptyEntries), int.Parse);

                // Create the tileset using the texture region
                Tileset tileset = new Tileset(textureRegion, tileWidth, tileHeight, blockingTiles);
                ...
                for (int row = 0; row < rows.Length; row++)
                {
                    ...
                    for (int column = 0; column < columnCount; column++)
                    {
                        ...
                        TextureRegion region = tileset.GetTile(tilesetIndex).Region;
