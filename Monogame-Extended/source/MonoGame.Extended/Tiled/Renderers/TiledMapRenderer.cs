...
namespace MonoGame.Extended.Tiled.Renderers
{
    public class TiledMapRenderer : IDisposable
    {
        ...
        private void Draw(TiledMapLayer layer, Vector2 parentOffset, Vector2 parentParallaxFactor, ref Matrix viewMatrix, ref Matrix projectionMatrix, Effect effect, float depth)
        {
            ...
            {
                // Handle TiledMapImageLayer with RepeatX/RepeatY
                if (layer is TiledMapImageLayer imageLayer)
                {
                    DrawImageLayer(imageLayer, offset, parallaxFactor, ref viewMatrix, ref projectionMatrix, effect, depth);
                }
                else
                {
                    DrawStandardLayer(layer, offset, parallaxFactor, ref viewMatrix, ref projectionMatrix, effect, depth);
                }
            }
        }

        private void DrawStandardLayer(TiledMapLayer layer, Vector2 offset, Vector2 parallaxFactor, ref Matrix viewMatrix, ref Matrix projectionMatrix, Effect effect, float depth)
        {
        ...
        private void DrawImageLayer(TiledMapImageLayer imageLayer, Vector2 offset, Vector2 parallaxFactor, ref Matrix viewMatrix, ref Matrix projectionMatrix, Effect effect, float depth)
        {
            var effect1 = effect ?? _defaultEffect;
            var tiledMapEffect = effect1 as ITiledMapEffect;
            if (tiledMapEffect == null)
            ...
            // Get the layer model for this image layer
            if (!_mapModel.LayersOfLayerModels.TryGetValue(imageLayer, out var layerModels))
                return;

            foreach (var layerModel in layerModels)
            {
                if (layerModel.Texture == null)
                    continue;
                ...
                // desired alpha
                tiledMapEffect.Alpha = imageLayer.Opacity;
