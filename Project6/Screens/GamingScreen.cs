...
namespace Project6.Scenes
{
    public class GamingScreen : GameScreen
    {
        ...
        public override void LoadContent()
        {
            ...
            GameObjectsManager.AddGameObject(spring);
            ...
            GameObjectsManager.AddGameObject(testObj);
        ...
        private void OnReadyThrowEgg(Vector2 position)
        {
            ...
            egg.OnOutOfBounds += () => {
                GameObjectsManager.RemoveGameObject(egg);
                GameObjectsManager.Player.CanThrowEgg = true;
            };
            egg.ScreenBounds = _sceneRenderer.GetScreenBounds();

            GameObjectsManager.AddGameObject(egg);
        }

        private void OnThrowEgg(Vector2 direction)
        {
            // 查找最近创建的蛋（或使用其他标识）
            Egg egg = GameObjectsManager.GetObjectsOfType<Egg>().LastOrDefault();

            if (egg != null && !egg.IsHeldAndThrew)
            {
                egg.ScreenBounds = _sceneRenderer.GetScreenBounds();
                egg.Throw(direction);
                GameObjectsManager.Player.CanThrowEgg = false;
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            // 更新场景渲染器（相机位置等）
            if (GameObjectsManager.Player != null)
            {
                GameObjectsManager.Player.ScreenBounds = _sceneRenderer.GetScreenBounds();
                _sceneRenderer.Update(gameTime, GameObjectsManager.Player.Position);
            }
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            _sceneRenderer.Draw(gameTime, GameObjectsManager.GetAllActiveObjects());
